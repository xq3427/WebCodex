...
const executionCodes = ['INVALID_EXECUTABLE', 'EXECUTABLE_NOT_FOUND', 'EXECUTION_PROFILE_CHANGED', 'CONCURRENCY_LIMIT'];
const stabilityCodes = ['FILE_OPERATION_RETRY_LIMIT','FILE_IMPORT_CONTENT_CHANGED','DOCUMENT_WIDGET_ASSET_INVALID','EXECUTION_UNKNOWN','BINARY_INPUT_INVALID','BINARY_INPUT_TOO_LARGE','BINARY_INPUT_INTEGRITY_ERROR','BINARY_CHUNK_INVALID','BINARY_CHUNK_CONFLICT','BINARY_CHUNK_ORDER','BINARY_CHUNK_EXPIRED','BINARY_CHUNK_CAPACITY','BINARY_CHUNK_INTEGRITY_ERROR','BINARY_CHUNK_BUSY'];
const safeCodes = new Set(['DEVICE_MISMATCH','WORKSPACE_NOT_FOUND','WORKSPACE_UNAVAILABLE','WORKSPACE_DISABLED','WORKSPACE_RESTART_REQUIRED','WORKSPACE_IDENTITY_MISMATCH','READ_ONLY','PATH_DENIED','NOT_FOUND','ACCESS_DENIED','ALREADY_EXISTS','FILE_BUSY','FILE_TOO_LARGE','BINARY_FILE','VERSION_CONFLICT','IDEMPOTENCY_CONFLICT','EXECUTION_DISABLED','EXECUTABLE_NOT_ALLOWED','INVALID_ARGUMENT','FILE_WIDGET_TICKET_NOT_FOUND','FILE_WIDGET_CACHE_FULL','FILE_WIDGET_EXPIRED','SERVICE_CLOSING','SESSION_STORE_INVALID','SESSION_STORE_DISABLED','SESSION_NOT_FOUND','SESSION_UNBOUND','SESSION_TURN_BUSY']);
for (const code of stabilityCodes) safeCodes.add(code);
for (const code of executionCodes) safeCodes.add(code);
for (const code of ['COPY_SAME_PATH', 'SOURCE_VERSION_CONFLICT']) safeCodes.add(code);
for (const code of ['RELAY_TIMEOUT', 'RELAY_READ_LIMIT', 'RELAY_EXPIRED', 'RELAY_NOT_FOUND', 'RELAY_CLOSED', 'RELAY_CANCELLED', 'RELAY_BUSY', 'RELAY_CAPACITY', 'RELAY_ACCESS_DENIED', 'RELAY_STALE_SUBMISSION', 'RELAY_INVALID_CONTENT', 'RELAY_PAYLOAD_TOO_LARGE', 'RELAY_INVALID_ARGUMENT']) safeCodes.add(code);
for (const code of ['DOCUMENT_INVALID_ARGUMENT','DOCUMENT_CAPACITY','DOCUMENT_UNSUPPORTED','DOCUMENT_EXPIRED','DOCUMENT_NOT_FOUND','DOCUMENT_PAGE_RANGE','DOCUMENT_READ_LIMIT','DOCUMENT_STALE_SUBMISSION','DOCUMENT_INVALID_RESULT','DOCUMENT_TEXT_LIMIT','DOCUMENT_ACCESS_DENIED','DOCUMENT_JOB_LIMIT','DOCUMENT_CACHE_LIMIT','DOCUMENT_RESULT_CONFLICT','DOCUMENT_ASSET_INVALID','DOCUMENT_ASSET_LIMIT','DOCUMENT_ASSET_UNAVAILABLE','PDF_INVALID','PDF_PASSWORD_REQUIRED','PDF_PARSE_FAILED','PDF_LIMIT','FILE_INTEGRITY_ERROR','ASSET_UNAVAILABLE']) safeCodes.add(code);

type Observation = { trace: string; started: number };
for (const code of ['FILE_IMPORT_SOURCE_DENIED','FILE_IMPORT_INVALID_ARGUMENT','FILE_IMPORT_PROXY_INVALID','FILE_IMPORT_DOWNLOAD_FAILED','FILE_IMPORT_TIMEOUT','FILE_IMPORT_INTEGRITY_ERROR','FILE_IMPORT_TOO_LARGE','FILE_IMPORT_BUSY','FILE_WRITE_VERIFICATION_FAILED']) safeCodes.add(code);
