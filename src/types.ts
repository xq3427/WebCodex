...
export interface AppConfig {
  ...
  fileBatches?: {maxFiles:number;maxTotalBytes:number;binaryMaxTotalBytes?:number};
  ...
  execution: { mode: 'disabled' | 'trusted-host'; commandPolicy?: 'all' | 'allowlist'; allowedExecutables: Record<string, ExecutableConfig>; env?: Record<string,string>; profiles?: Record<string, ExecutionProfile>; maxConcurrent: number; defaultTimeoutMs?: number; maxTimeoutMs: number; maxOutputBytes: number; defaultWaitMs?:number;maxWaitMs?:number;stdinMaxBytes?:number;stdinMaxTotalBytes?:number;stdinWriteTimeoutMs?:number };
  limits: { readMaxBytes: number; fileReadMaxBytes?: number; fileTransferMaxBytes?: number; fileWidgetUploadMaxBytes?: number; fileWidgetTicketTtlMs?: number; fileWidgetCacheMaxBytes?: number; fileWidgetChunkMaxBytes?: number; binaryWriteMaxBytes?: number; inlineBinaryWriteMaxBytes?: number; writeMaxBytes: number; searchMaxResults: number; listMaxEntries: number };
  rgPath: string;
