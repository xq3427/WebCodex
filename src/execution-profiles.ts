...
/** Select one local environment. A profile replaces, rather than extends, global aliases and variables. */
export function effectiveWorkspaceExecution(config: AppConfig, workspaceId: string): EffectiveWorkspaceExecution {
  ...
  const allowedExecutables = Object.fromEntries(Object.entries(selected.allowedExecutables).map(([alias, value]) => {
  ...
  return { profile, allowedExecutables, env: normalizeExecutionEnvironment(selected.env) };
}
