...
本指南对应 0.16.0-preview.18、配置 schema v2。主线使用 OpenAI 官方 Secure MCP Tunnel 连接本地 MCP。`connect` 默认同时启动本机配置控制中心，并输出带临时凭据的完整本机链接，可直接在页面管理本次连接。本地 PDF 文字层使用 document_open/document_read，生成文件自动回存使用 fs_save_file/status，均需分别通过实际宿主验收；不等同于自动上传为原生附件。
...
> 使用 WebCodex，实际调用 system_status 和 workspace_list。确认版本为 0.16.0-preview.18，报告目标设备和工作区；读取我指定工作区的一个已知文本文件。不要仅凭应用详情判断工具可调用。
