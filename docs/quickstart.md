...
推荐下载 [0.16.0-preview.18 安装包](https://github.com/xq3427/WebCodex/releases/download/v0.16.0-preview.18/WebCodex-0.16.0-preview.18-setup.zip)。它包含已构建的 WebCodex，无需克隆仓库或编译。首次安装需要联网访问 GitHub、npm registry；缺少 Node 时还会访问 nodejs.org。安装器不要求 npm 登录。
...
```sh
curl -fL -o WebCodex-setup.zip https://github.com/xq3427/WebCodex/releases/download/v0.16.0-preview.18/WebCodex-0.16.0-preview.18-setup.zip
...
> 使用 WebCodex，先调用 system_status 和 workspace_list，确认版本为 0.16.0-preview.18、设备与工作区正确。读取 README.md（如果存在）。在我指定的可写工作区新建一个不存在的 webcodex-smoke.txt，内容为“连接测试”，再实际读回。不要覆盖已有文件；失败时报告实际工具错误。
...
已有 Node.js ≥22.16 和 npm，可直接使用 [npm 上的 0.16.0-preview.18](https://www.npmjs.com/package/webcodex-mcp/v/0.16.0-preview.18)，无需 npm 账号或登录。先在源码仓库及其子目录之外新建专用目录，再运行；例如在用户主目录创建：

需要在任意目录直接使用 `webcodex init` / `webcodex connect` 时，执行全局安装：
...
首次 `init` 在你输入 Tunnel ID 和 API key 后会自动安装并校验官方 tunnel-client；如果配置已存在，`init` 会保留原配置，不会覆盖凭据。升级旧安装后若仍提示 `TUNNEL_CLIENT_INVALID`，请重新运行 `webcodex init --config <现有配置路径>` 或在控制面板中检查 `toolsDir`。

普通 `npm install webcodex-mcp` 是当前项目内安装，不会保证 `webcodex` 进入系统 PATH；应改用 `npx webcodex` 或上述 `-g` 安装。Windows 全局安装后若当前 PowerShell 仍提示“无法识别”，重新打开终端并用 `npm prefix -g` 确认该目录已加入用户 PATH。

```sh
mkdir ~/WebCodex-local
cd ~/WebCodex-local
npx --yes --package webcodex-mcp@0.16.0-preview.18 webcodex setup --workspace ./workspace --config ./config.toml
```
...
也可以下载 Release 中的 `webcodex-mcp-0.16.0-preview.18.tgz`，在专用目录运行：

```sh
npm install ./webcodex-mcp-0.16.0-preview.18.tgz --omit=dev --ignore-scripts
