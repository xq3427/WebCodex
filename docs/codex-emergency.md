...
在唯一的本机配置中设置 `codexSessions.home` 并启用专用只读会话工具。该目录可以位于任意本机磁盘；会话工具仅访问日志和索引，不开放整个 Codex 目录。普通文件/Git 工具仍拒绝配置、认证文件和 Codex home，只有本机明确登记、位于 home/worktrees 严格后代的选定 linked worktree 可按工作区授权访问。新安装的历史读取和命令执行均默认关闭；已有配置不会因升级自动扩权。
