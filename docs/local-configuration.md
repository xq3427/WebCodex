...
| `localPanel` | 本地只读面板端口 port，默认 8767；固定监听 127.0.0.1 |
...
| `limits` | 文本读取、源文件/哈希预算、有界 MCP 原字节和诊断组件、写入、搜索及目录列表限额；不限制原生附件大小 |
...
面板与 `connect` 选择同一配置，但单独运行，不初始化 App/StateStore 或启动第二个 daemon。它只读查看设备/工作区、官方隧道状态、作业与输出、Codex 会话索引，并提供原文件路径和打开所在目录。

启动 URL 的 fragment 中包含本次面板凭据，需保密；页面清除地址片段后仅使用本机会话 `sessionStorage`，不加载外部资源。凭据随面板进程停止/重启失效，不从配置 API key 派生。修改配置后分别重启需要加载新设置的面板与隧道，再打开新面板 URL。详见[面板指南](local-panel.md)。
...
以下设置仅为保留的 `fs_open_file / file_widget_probe` 显式诊断流程提供兼容，不是正常阅读文档的必配项，也不控制本地面板或 ChatGPT 原生附件流程。当前仍有 44 个兼容工具，其中两个分块工具只供组件调用。
...
无文件探针始终手动。原文件传入、哈希、组件上传回执与模型实际读到正文是不同结果；此前组件上传后正文仍不可访问，不能根据这些设置或 fileId 宣称功能已完成。日常文档阅读使用原生附件，诊断历史见[原文件说明](original-files.md#保留的工具)。
...
| 字段 | 默认值 | 允许范围与含义 |
|---|---|---|
| `fileBatches.maxFiles` | 20 | 1–100；每批涉及的不同路径数，移动计源/目标两条路径 |
| `fileBatches.maxTotalBytes` | 4194304 | 1024–33554432；一批全部修改前后原始字节的合计 |
...
批次中的单个文件仍受 `limits.writeMaxBytes` 约束，不能通过拆成批量操作绕过；预览 diff 的正文预算受 `limits.readMaxBytes` 约束。文件数量、备份及修改后字节全部预检，预览本身不保存计划或锁定外部编辑器。详见[批量变更](file-workflow.md#多文件预览与应用)。
...
执行默认关闭。`trusted-host` 使用服务用户权限，并不是 OS 沙箱。只接受本机配置的 native 程序别名；`.cmd/.bat` 不直接执行，npm 可配置 Node 程序并用 `--prefix-arg /absolute/npm-cli.js` 添加固定入口。固定参数支持重复，参数以 `--` 开头时使用 `--prefix-arg=--flag`。程序路径由每台设备独立配置。

`execution inspect` 只检查原生程序文件、可识别的入口文件和配置变量名，不运行解释器、版本命令或项目脚本，也不打开 WebCodex state。返回 `static_check_only: true`；顶层 `ready` 的范围是 `global_defaults`，应同时检查 `workspaces` 中选定工作区的 profile、可用性和 ready。ready 为真只代表对应静态条件通过，不能证明运行时身份、构建依赖、解释器版本或脚本可正常运行。输出不显示固定参数和环境变量值。
...
本机 HTTP 使用 `server.transport="http"`，同一文件中的 `http.bearerToken` 至少 32 字符（init 自动生成，公开模板留空）；它不是用于公开 ChatGPT 连接的 OAuth。配置修改后手动重启服务生效。
